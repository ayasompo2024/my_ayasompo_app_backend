    ...
    <div class="container">
        ...
        <nav aria-label="breadcrumb m-0 p-0">
            <ol class="breadcrumb m-0 pl-0 bg-transparent">
                <li class="breadcrumb-item active p-0 pl-1" aria-current="page">Product / News</li>
            </ol>
        </nav>
        ...
        <div class="row mt-2">
            <div class=" col-md-12 px-md-5 rounded mb-5">
                <h5 class="mt-4 pb-2 border-bottom">
                    <span class="float-right"> Add New Product</span>
                    <a href="{{ url()->previous() }}"><i class="bi bi-arrow-left-square"></i></a>
                </h5>
                <form class="mt-4" method="POST" action="{{ route('admin.product.store') }}"
                    enctype="multipart/form-data" onkeydown="return event.key != 'Enter'">
                    @csrf
                    @include('admin.product.partials._create_product_form')
                    @include('admin.product.partials._create_thumbnail_form')
